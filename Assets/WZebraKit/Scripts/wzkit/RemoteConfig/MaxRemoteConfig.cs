...
{
    public class MaxRemoteConfig : ILionSettingsProvider
    {
        public string Name => "RemoteConfig";
        ...
        public static MaxRemoteConfig.RemoteConfigSettings lionSettings =  new MaxRemoteConfig.RemoteConfigSettings();
        ...
        public static T LoadRemoteData<T>(string variableName)
        {
            ...
            if (!string.IsNullOrEmpty(remoteValStr))
            {
                ...
                LionDebug.Log($"RemoteConfig: Remote variable found from server! : Value Found = {remoteValStr}");
            ...
            {
                LionDebug.Log($"RemoteConfig: The server has returned a null value for [{variableName}]." +
                    "Please check that Lion Settings variable name matches the MAX Experiments variable name exactly and that the MAX Experiements variable has a valid value");
            ...
            LionDebug.Log($"RemoteConfig: {variableName} is not defined in Lion Settings Remote Configuration and unable to request this variable from the Max server");
            return (T) default;
        }
